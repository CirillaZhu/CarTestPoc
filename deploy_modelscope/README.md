...
## 使用
- **问答**页签：直接输入问题，例如「F3级前雾灯的明暗截止线有什么要求？」，
  下方给出答案、关联的图片/表格、以及检索来源。
- **知识库地图**页签：每个点是一个知识块、颜色按来源文档区分。勾选 / 取消某文档即
  「新增 / 撤下」该文档，可直观看到点簇的出现与覆盖范围；把问题投到图上还能对比
  新增前后的检索命中与相似度，演示「知识库维护的过程与效果」。
